...
Cohesion: 0.14
Nodes (13): bos_choch(), detect_swings(), displacement(), Market structure: vectorized swing detection, BOS, CHoCH.  Tick-precision: when, Detect displacement candles (body > mult * ATR)., Fractal swing detection.      Returns (swing_high_idx, swing_high_px, swing_low_, Compute BOS and CHoCH events without look-ahead.      A swing at index j is only, _ctx_or_compute() (+5 more)
...
Cohesion: 0.18
Nodes (10): Volume Spread Analysis (VSA) — Tom Williams style.  Detects:   - No Demand bar:, Returns (idx, code).     code: 1=no_demand, -1=no_supply, 2=stopping_volume_bull, vsa_signals(), ConfluenceScorer, default(), Pluggable confluence scorer.  Idea ---- Each *scorer* is a stateless function th, Volume Spread Analysis confluence.      Maps VSA codes from `vsa_signals` to bul, Same return shape as confluence.build_signals. (+2 more)
